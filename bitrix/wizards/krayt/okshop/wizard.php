...
class DemoForm extends CWizardStep
{
    function InitStep()
    {
        $this->SetStepID("demo_form");
        $this->SetTitle(GetMessage("WIZ_STEP_DEMO_FORM"));
    ...
    function ShowStep()
    {
        ...
        $package =& $wizard->GetPackage();
        ob_start();
        include($_SERVER["DOCUMENT_ROOT"].$package->GetPath()."/html/form.php");
        $html = ob_get_clean();
        if($html)
        {
            $this->content.= $html;
