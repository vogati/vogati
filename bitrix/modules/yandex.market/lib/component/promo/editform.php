...
class EditForm extends Market\Component\Model\EditForm
{
	...
	protected function getTypeDiscountOverrides($promoType)
	{
		...
		switch ($promoType)
		{
			...
			default:
				$result = [
					'EXTERNAL_ID' => [ 'MANDATORY' => 'Y' ]
				];

				if (!Market\Export\Promo\Discount\Manager::isInternalType($promoType))
                {
                    try
                    {
                        ...
                        $result['EXTERNAL_ID']['DESCRIPTION'] = $providerClassName::getDescription();

                        if ($providerEnum !== null)
                        {
                            $result['EXTERNAL_ID']['USER_TYPE'] = $USER_FIELD_MANAGER->GetUserType('enumeration');
                            $result['EXTERNAL_ID']['USER_TYPE']['CLASS_NAME'] = 'Yandex\Market\Ui\UserField\EnumerationType';
