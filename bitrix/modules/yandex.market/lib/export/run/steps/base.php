...
abstract class Base
{
	...
	protected function updateDataStorage($filter, $fields)
	{
		...
		{
			$updateResult = $dataClass::updateBatch([ 'filter' => $filter ], $fields);
	...
	protected function resolveStoredCollision($tagResultList, $fieldsList, $context)
	{
		...
		foreach ($collisionFieldsMap as $collisionField => $useCollision)
		{
			...
			{
				if ($status === static::STORAGE_STATUS_INVALID)
				{
	...
	protected function checkStoredCollision($valueMap, $fieldName, $context)
	{
		...
		if ($dataClass && !empty($valueMap))
		{
			...
			foreach (array_chunk($valueMap, 500, true) as $valueChunk)
			{
				$query = $dataClass::getList([
					'filter' => $filter + [
						'!=ELEMENT_ID' => array_values($valueChunk),
						'=STATUS' => [
							static::STORAGE_STATUS_SUCCESS,
							static::STORAGE_STATUS_INVALID,
						],
