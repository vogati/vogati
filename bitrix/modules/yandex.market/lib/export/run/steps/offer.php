...
class Offer extends Base
{
	...
	protected function makeQueryFilter($sourceFilterList, $sourceSelectList, $queryContext, $changesFilter = null)
	{
		...
		$hasOffers = !empty($iblockIds['OFFERS']);
		...
		if ($hasOffers && !empty($result['OFFERS']) && empty($result['DISTINCT']) && empty($result['ELEMENT']))
		{
		...
		{
			$result = $this->appendQueryFilterChanges($result, $changesFilter, $iblockIds, $queryContext);
		}
		...
		if ($hasOffers)
		{
			...
			else if (!$isOfferSubQueryInitialized && $result['DIRECTION'] !== 'OFFER' && !$this->isIgnoreOfferSubQuery($queryContext))
			{
				$result['ELEMENT'][] = [
					'ID' => \CIBlockElement::SubQuery(
						'PROPERTY_' . $queryContext['OFFER_PROPERTY_ID'],
	 ...
	 * @param $iblockIds array<string, int|null>
	 * @param $queryContext array
	 *
	 * @return array<string, array>
	 */
	protected function appendQueryFilterChanges($resultFilter, $changesFilter, $iblockIds, $queryContext)
	{
		foreach ($changesFilter as $entityType => $entityFilter)
		{
			...
			{
				$resultFilter[$entityType] = $this->getFilterDefaults($entityType, $iblockIds[$entityType]);
			}
