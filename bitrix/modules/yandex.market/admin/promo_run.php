...
?>
    <form class="js-plugin" action="<?= $APPLICATION->GetCurPage() . '?lang=' . LANGUAGE_ID; ?>" method="post" data-plugin="Ui.Admin.ExportForm">
        ...
        if ($isShowPromoChoose)
		{
			...
			?>
			<tr>
				<td width="40%" align="right" valign="top"><?= Market\Config::getLang('ADMIN_PROMO_RUN_FIELD_PROMO_ID'); ?>:</td>
				<td width="60%">
		...
		if (!$isDeleteInactiveByDefault)
		{
			?>
			<tr>
				<td width="40%" align="right" valign="top"><?= Market\Config::getLang('ADMIN_PROMO_RUN_FIELD_DELETE_INACTIVE'); ?>:</td>
				<td width="60%">
        ...
        ?>
        <input type="button" class="adm-btn adm-btn-save js-export-form__run-button" value="<?= Market\Config::getLang('ADMIN_PROMO_RUN_BUTTON_START'); ?>" <?= $accessLevel < 'R' ? 'disabled' : ''; ?> />
