(function(BX, $, window) {
	...
	var constructor = Condition.Item = Reference.Base.extend({
		...
		defaults: {
			...
			valueAutocompleteTemplate: '<select class="b-filter-condition-field__input js-condition-item__input js-condition-item__value js-plugin" size="1" data-plugin="Ui.Input.TagInput" data-autocomplete="true" data-type="autocomplete"></select>',
		...
		refreshValue: function(fieldValue, compareValue) {
			...
			valueElement = this.updateValue(valueList, isMultiple, definedValue, isAutocomplete);

			valueElement.data('sourceField', fieldValue);
			valueElement.data('sourceCompare', compareValue);
			valueElement.data('iblockId', this.getIblockId(manager));
		},

		updateValue: function(enumList, isMultiple, definedValue, isAutocomplete) {
