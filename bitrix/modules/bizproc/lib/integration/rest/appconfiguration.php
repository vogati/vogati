	...
	private static function importCrmTrigger($importData)
	{
		...
		)
		{
			if(isset($importData['RATIO']['CRM_STATUS']))
			{
				...
				{
					$item['APPLY_RULES'] = static::changeDealCategory($item['APPLY_RULES'], $importData['RATIO']['CRM_STATUS']);
				}
				$item['ENTITY_STATUS'] = static::changeDealCategory($item['ENTITY_STATUS'], $importData['RATIO']['CRM_STATUS']);
			}
